...
public:
  ...
  uint32_t calculateMessageIdentifier()
  {
    ...
    {
      c = 1;
    } //messageIdentifier is never zero
    return ret;
  }
  ...
  void removeItem()
  { //Cleaning db  --> Remove the oldest item
...
void espNowFloodingMesh_loop()
{
  ...
  if (masterFlag)
  {
    ...
    if (elapsed >= RESEND_SYNC_TIME_MS)
    { //10s
      start = millis();
#ifdef DEBUG_PRINTS
...
#endif
{
  ...
  if (len >= sizeof(struct meshFrame))
    return;
...
#endif
  if (!(m.encrypted.header.msgId == USER_MSG || m.encrypted.header.msgId == SYNC_TIME_MSG || m.encrypted.header.msgId == INSTANT_TIME_SYNC_REQ || m.encrypted.header.msgId == USER_REQUIRE_RESPONSE_MSG || m.encrypted.header.msgId == USER_REQUIRE_REPLY_MSG))
  {
    //Quick wilter;
    return;
  }
  if (m.encrypted.header.length >= 0 && m.encrypted.header.length < (sizeof(m.encrypted.data)))
  {
    ...
    if (crc16 == crc)
    {
      ...
      bool ok = false;
      if (messageStatus == 0)
      { //if messageStatus==0 --> message is not handled yet.
        ...
        if (m.encrypted.header.msgId == SYNC_TIME_MSG)
        {
          ...
          Serial.println(last_time_sync);
          Serial.print("Sync time in message:");
          Serial.println(m.encrypted.header.time);
          ...
          if (last_time_sync < m.encrypted.header.time || ALLOW_TIME_ERROR_IN_SYNC_MESSAGE)
          {
            ...
            Serial.print(asctime(localtime(&currentTime)));
            //  #endif
    ...
    else
    {
      ...
      Serial.print("#CRC: ");
      Serial.print(crc16);
      Serial.print(" "), Serial.println(crc);
...
{
  if (masterFlag || timeStampCheckDisabled)
    return true;
  syncronized = false;
  for (int i = 0; i < tryCount; i++)
  {
